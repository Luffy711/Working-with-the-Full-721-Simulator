...
private:
	...
	uint64_t n_log_regs, n_phys_regs, n_branches, n_active, n_free;
	...
	//
	// Notes:
	// * Structure includes head, tail, and and their phase bits.
	/////////////////////////////////////////////////////////////////////
	...
	struct Active_List
	{
		...
		bool completed, exception, load_violation, branch_misprediction, value_misprediction;
		bool destination_flag, load_flag, store_flag, branch_flag, amo_flag, csr_flag;
		uint64_t PC; 
	};
	vector<Active_List> active_list;
	...
	// 2. checkpointed Free List head index and its phase bit
	// 3. checkpointed GBM
	/////////////////////////////////////////////////////////////////////
	...
	struct checkpoints
	{
		vector<uint64_t> SMT;
		uint64_t head;
		uint64_t recover_GBM;
	};
	vector<checkpoints> branch_checkpoint;
	...
	void copy_state(vector<uint64_t>& dest, vector<uint64_t>& source)
	{
...
public:
	...
	renamer(uint64_t n_log_regs_, uint64_t n_phys_regs_, uint64_t n_branches_, uint64_t n_active_);
